...
import { CreateGradeAssessmentDto } from '../dto/grade-assessment.dto';
...
@Injectable()
export class CreateGradeAssessmentUseCase {
  constructor(
    private readonly gradesAssessmentsRepository: GradesAssessmentsRepository,
    private readonly authRepository: AuthRepository,
  ) {}

  async execute(command: CreateGradeAssessmentDto) {
    ...
    const { data, normalized } = buildCreateAssessmentData({
      scope,
      academicYearId,
      command,
      assessmentScope,

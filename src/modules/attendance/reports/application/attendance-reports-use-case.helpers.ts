...
import { AttendanceReportBaseQueryDto } from '../dto/attendance-reports.dto';
import {
...
export async function buildAttendanceReportFilters(
  repository: AttendanceReportsRepository,
  query: AttendanceReportBaseQueryDto,
): Promise<AttendanceReportFilters> {
  await validateOptionalAttendanceReportContext(repository, query);

  const resolvedScope = await resolveOptionalAttendanceReportScope(
    repository,
    query,
  );
  const dates = resolveAttendanceReportDateFilters(query);
...
export function formatAttendanceReportDate(value: Date): string {
  return value.toISOString().slice(0, 10);
}

async function validateOptionalAttendanceReportContext(
  repository: AttendanceReportsRepository,
  query: AttendanceReportBaseQueryDto,
): Promise<void> {
...
async function resolveOptionalAttendanceReportScope(
  repository: AttendanceReportsRepository,
  query: AttendanceReportBaseQueryDto,
  ...
  return resolveAttendanceReportScope(repository, {
    scopeType: query.scopeType,
...
async function resolveAttendanceReportScope(
  repository: AttendanceReportsRepository,
...
function resolveAttendanceReportDateFilters(
  query: AttendanceReportBaseQueryDto,
): {
  date?: Date;
  dateFrom?: Date;
...
function extractScopeIds(input: ReportScopeInput): AttendanceScopeIds {
  return {
    stageId:
      input.stageId ??
...
function rejectIds(
  ...
  if (present.length > 0) {
    throw new ValidationDomainException(
      'Attendance report scope contains ids that do not match its scope type',
      { fields: present },
    );
  }
}
...
function requireId(
  ...
  if (!value) {
    throw new ValidationDomainException(
      `${scopeType} attendance report scope requires ${field}`,
      { scopeType, field },
    );
  }
...
function assertOptionalParent(
  ...
  if (provided && provided !== actual) {
    throw new ValidationDomainException(
      'Attendance report scope parent ids do not match the selected scope',
      { field },
    );
  }
}

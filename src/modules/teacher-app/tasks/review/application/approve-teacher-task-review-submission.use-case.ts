...
@Injectable()
export class ApproveTeacherTaskReviewSubmissionUseCase {
  constructor(
  ...
  async execute(
    submissionId: string,
    dto: ApproveTeacherTaskReviewSubmissionDto,
    ...
    await this.approveReinforcementSubmissionUseCase.execute(submissionId, {
      note: normalizeText(dto.note) ?? normalizeText(dto.comment),

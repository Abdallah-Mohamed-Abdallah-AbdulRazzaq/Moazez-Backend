...
export const PARENT_MESSAGE_SEND_TYPES = [
  'text',
  'image',
  'file',
  'audio',
...
export const PARENT_MESSAGE_CONTACT_ROLES = [
  'teacher',
  'student',
  'parent',
] as const;
...
export class SearchParentConversationMessagesQueryDto {
...
export class ParentMessageReadersQueryDto {
  @IsOptional()
  @Type(() => Number)
  @IsInt()
  @Min(1)
...
export class ListParentMessageContactsQueryDto {
  @IsOptional()
  @IsString()
  @MaxLength(80)
  q?: string;
  ...
  @IsOptional()
  @Transform(({ value }) => toLowerOptionalString(value))
  @IsIn(PARENT_MESSAGE_CONTACT_ROLES)
  role?: string;
...
export class CreateParentMessageConversationDto {
  @IsString()
  @MaxLength(128)
  contactId!: string;
}

export class SendParentConversationMessageAttachmentDto {
  @IsUUID()
  fileId!: string;
...
export class SendParentConversationMessageDto {
  @IsOptional()
  @Transform(({ value }) => toLowerOptionalString(value))
  @IsIn(PARENT_MESSAGE_SEND_TYPES)
  type?: string;
  ...
  @Type(() => SendParentConversationMessageAttachmentDto)
  attachments?: SendParentConversationMessageAttachmentDto[];
}
...
export class ParentMessageContactDto {
  contactId!: string;
  contact_id!: string;
  displayName!: string;
  display_name!: string;
...
export class ParentMessageContactsResponseDto {
  contacts!: ParentMessageContactDto[];
...
export class ParentMessageReaderDto {
  userId!: string;
  user_id!: string;
  displayName!: string;
  display_name!: string;
...
export class ParentMessageAttachmentDto {
  attachmentId!: string;
...
export class ParentMessageDto {
  ...
  readCount!: number;
  read_count!: number;
  attachments!: ParentMessageAttachmentDto[];
  attachmentsCount!: number;
  attachments_count!: number;
  audio_url!: null;
  audio_duration!: null;
...
export class ParentConversationMessageSearchResponseDto extends ParentConversationMessagesResponseDto {
  query!: string;
}
...
export class ParentMessageReadersResponseDto {
  messageId!: string;
  message_id!: string;
  conversationId!: string;
  conversation_id!: string;
  ...
  fullyRead!: boolean;
  fully_read!: boolean;
  readers!: ParentMessageReaderDto[];
  pagination!: ParentMessagePaginationDto;
}

export class ParentMessageInfoMessageDto {
  messageId!: string;
  message_id!: string;
  conversationId!: string;
  conversation_id!: string;
...
export class ParentMessageInfoResponseDto {
  message!: ParentMessageInfoMessageDto;
  readers!: ParentMessageReaderDto[];
  readCount!: number;
  read_count!: number;
  participantsCount!: number;

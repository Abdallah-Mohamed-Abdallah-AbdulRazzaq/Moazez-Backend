...
Update note - 2026-07-08:

- This audit is historical. It captured the state before the subject-allocation matrix existed.
- The current backend now has `SubjectAllocation` as the term/grade/subject/weekly-hours source of truth.
- `Subject` remains a school-scoped catalog item only. The subject create/update DTOs and subject response no longer expose `termId` or `stage`.
- Current frontend guidance for this contract is in `docs/sprint-academics-subjects-catalog-allocation-contract-1a-closeout.md`.

At the time of the original audit, Academics was not complete. The backend had a solid core for structure, subjects, rooms, calendar, overview, curriculum, lesson content, lesson plans, timetable, schedule read models, homework, and grades. Since then, subject allocation weekly hours have been implemented separately from the subject catalog. Remaining completion gaps should be assessed from current closeout docs and tests rather than this historical audit alone.
...
| Subjects                                         | COMPLETE for catalog contract                                    | School-scoped subject CRUD exists. Subject create/update is catalog-only and rejects `termId`/`stage` through validation. Subject responses no longer present `termId` or `stage`.                                                          | Use `academics/subject-allocations` for term/grade/weekly-hours assignment. Do not treat `/academics/subjects` as a term subject endpoint.                                                                                                                                                   | `src/modules/academics/subjects/controller/subjects.controller.ts`, `src/modules/academics/subjects/dto/subject.dto.ts`, `src/modules/academics/subjects/dto/subject-response.dto.ts`, `src/modules/academics/subjects/presenters/subjects.presenter.ts`, `prisma/schema.prisma`                                                                                                                       |
| Subject allocation matrix / weekly hours         | COMPLETE for current V1 matrix                                   | `SubjectAllocation` stores `(academicYearId, termId, gradeId, subjectId, weeklyHours)` and is exposed through `GET /api/v1/academics/subject-allocations` and `PUT /api/v1/academics/subject-allocations/bulk`.                             | This is the source of truth for term/grade subject assignment. Stage is not an allocation key; gradeId is the allocation key.                                                                                                                                                                | `src/modules/academics/subject-allocation/**`, `prisma/schema.prisma`, `test/e2e/academics-subject-allocations.e2e-spec.ts`, `test/security/tenancy.academics-subject-allocations.spec.ts`                                                                                                                                                                                                             |
...
- `SubjectAllocation` now links term, grade, subject, and weekly hours for the dashboard subject matrix.
- `GET /api/v1/academics/subject-allocations?termId=...&gradeId=...` reads allocated subjects.
- `PUT /api/v1/academics/subject-allocations/bulk` writes allocation rows.

Gap:

- The basic allocation matrix gap recorded by the original audit has been resolved.
- Future teacher load analytics and timetable readiness can consume `SubjectAllocation`.
- `/api/v1/academics/subjects` remains catalog-only and must not be used as a term/grade allocation endpoint.

Classification:

- Historical product gap, now implemented for the current V1 matrix.
- Current contract risk is frontend confusion between catalog subjects and allocated term/grade subjects.
...
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Subject allocation matrix / weekly hours | `SubjectAllocation` model and `weeklyHours` field exist.                                                                             | No new schema need for the catalog/allocation contract cleanup. Keep using the dedicated `(schoolId, termId, gradeId, subjectId)` allocation model.  | `prisma/schema.prisma`, `src/modules/academics/subject-allocation/**`, `adr/School-Dashboard/sis_dashboard-academics_backend_handoff_spec.md`                                        |
...
- This plan has since been implemented for the current V1 matrix.
- The accepted route namespace is `/api/v1/academics/subject-allocations`.
- Do not move this behavior under `/api/v1/academics/subjects`.
...
| ------------------------------------------------------------------------------------ | ---------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Subject catalog/allocation confusion                                                 | Contract clarity risk        | Medium   | Keep `/academics/subjects` catalog-only and use `/academics/subject-allocations` for term/grade/weekly-hours assignment. | `src/modules/academics/subjects/**`, `src/modules/academics/subject-allocation/**`, `docs/sprint-academics-subjects-catalog-allocation-contract-1a-closeout.md` |
...
- Subject allocation matrix with weekly hours. Completed after the original audit for the current V1 matrix; keep catalog/allocation contracts separate.

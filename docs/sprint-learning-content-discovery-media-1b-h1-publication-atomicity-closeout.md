...
### CI pool-observer correction

GitHub Actions initially exposed test-observer connection-pool starvation in the atomicity suite while the primary Prisma pool was constrained to `connection_limit=3` and `pool_timeout=10`. The gate transaction, actual parent-delete transaction, and actual Lesson Content publish/create transaction occupied the three primary connections, so `waitUntilBlockedBackendCount` could not acquire a fourth connection for its `pg_stat_activity` query. This was test instrumentation failure, not a production deadlock or cascade-contract failure.

PostgreSQL lock observation now uses a dedicated test-only `PrismaClient` with its own `connection_limit=1` and `pool_timeout=10`; fixtures, repositories, production use cases, gate transactions, application transactions, audits, and cleanup remain on the unchanged primary client. The CI-equivalent red reproduction failed the six parent-first cases with pool timeouts (54/60 passed), then three separate corrected atomicity runs passed 60/60 each with the primary pool still limited to three connections. The complete PostgreSQL integration group passed 3/3 suites and 85/85 tests, and the focused unit/contract/controller group passed 5/5 suites and 81/81 tests. Build, migration governance, Prisma validation, ESLint, Prettier, and diff validation passed. No production, workflow, schema, migration, package, or lockfile change was made by this correction. A subsequent GitHub Actions result is not claimed because no commit or push was authorized.

...
- Push provider delivery depends on existing Firebase/FCM deployment credentials. When the provider is disabled or dry-run, delivery rows/attempts are marked skipped by existing communication push infrastructure.
...
- Push/device-token completion adds one enum-only migration:
  `20260706170000_dismissal_staff_device_token_surface`.
...
- Push delivery rows are created after committed Dismissal notification rows and enqueued through the existing communication push queue.
- Push payloads include only safe title/body/data navigation hints and never include pickup codes, pickup-recipient tokens, parent coordinates, raw metadata, or token material.

...
describe('PARENT-DISMISSAL-1A route metadata and seed boundaries', () => {
  ...
  it('adds only the read permission to Parent role and no smart-pickup request/cancel or dismissal permissions', () => {
    ...
    expect(parentPermissions).not.toContain('parent.smart_pickup.request');
  ...
  it('does not add request/device-token schema for this read-only sprint', () => {
    const schemaSource = readFileSync('prisma/schema.prisma', 'utf8');

    expect(schemaSource).not.toMatch(/model\s+DismissalRequest\b/);
    expect(schemaSource).not.toMatch(/model\s+DismissalRequestEvent\b/);
...
describe('PARENT-DISMISSAL-1A tenancy and RBAC (security)', () => {
  ...
  it('does not expose deferred request/root routes', async () => {
    await request(app.getHttpServer())
      .post(`${GLOBAL_PREFIX}/parent/smart-pickup/requests`)
      .set('Authorization', `Bearer ${parentAToken}`)
      .send({ childId: ownedChildId })
      .expect(404);
    await request(app.getHttpServer())

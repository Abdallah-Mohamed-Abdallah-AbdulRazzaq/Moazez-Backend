...
test('Stage 29A maintenance range preserves local working-tree and linear-commit fallback', () => {
...
test('Stage 29A maintenance range uses the exact PR base and single-parent candidate', () => {
...
test('Stage 29A merge maintenance excludes files imported from the base branch', () => {
  ...
  assert.deepEqual(
    actual.filter((file) =>
      PRODUCTION_API_CAPACITY_REMEDIATION_PATHS.includes(file),
    ),
    [],
  );

...
test('historical G07 mode retains its frozen identity and allowlist', () => {
  ...
  assert.equal(
    ALLOWED_SCOPE_PATHS.has('src/runtime/core-worker/core-worker-runtime.module.ts'),
    false,
  );
});
...
test('current mode rejects tracked env files without reading their contents', async (t) => {
  ...
  fs.writeFileSync(path.join(repository.root, '.env.production'), 'DO_NOT_READ=1\n');
  git(repository.root, 'add', '.env.production');
  git(repository.root, 'commit', '-m', 'track env fixture');

  const result = await runScopeCheck(scopeContext(repository.root, repository.base));
...
test('current mode records changed paths without a phase allowlist', async (t) => {
  ...
  assert.equal(context.comparison.head, git(repository.root, 'rev-parse', 'HEAD'));
...
test('workflow preserves the blocking identity and runs the neutral current aggregate', () => {
  const repositoryRoot = path.resolve(__dirname, '../..');
  const workflow = fs.readFileSync(
    path.join(
      ...
      '.github',
      'workflows',
      'phase-1-universal-regression.yml',
    ),
    'utf8',
  );
  ...
  assert.match(workflow, /run: npm run test:regression/u);
  assert.match(workflow, /universal-regression-summary-\$\{\{ github\.run_id \}\}/u);
...
function createGitRepository(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'moazez-regression-contract-'));
